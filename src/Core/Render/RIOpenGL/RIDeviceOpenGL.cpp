...
namespace Phoenix
{
	...
	{ 
		...
		texture->m_dataType = GL_UNSIGNED_BYTE; // NOTE(Phil): This might be problematic with higher depth textures later (hdr)

		glGenTextures(1, &texture->m_id);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(texture->m_pixelFormat, texture->m_id);
		...
		if (checkGlErrorOccured())
		{
			Logger::error("Failed to compile create Texture2D!");
